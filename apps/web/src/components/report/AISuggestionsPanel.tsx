        ...
        {suggestions.bullet_rewrites.map((rewrite, i) => (
          <div key={i} className="border border-[var(--color-border)] rounded-xl overflow-hidden">
            ...
            >
              ...
              <ChevronRight size={14} className={`flex-shrink-0 transition-transform ${expandedIndex === i ? "rotate-90" : ""}`} />
            </button>
            {expandedIndex === i && (
              <div className="px-4 pb-4 space-y-2">
                <div>
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full text-[var(--color-danger)]" style={{ backgroundColor: "var(--color-danger-soft)" }}>Original</span>
                  <p className="text-xs text-[var(--color-muted)] mt-1.5">{rewrite.original}</p>
                </div>
                <div>
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full text-[var(--color-success)]" style={{ backgroundColor: "var(--color-success-soft)" }}>Suggested</span>
